...
| **설명**      | 손가락을 모두 모아 기본 상태를 유지합니다. <br/>이때 반응형 버튼(인사·감동·웃음·놀람)을 사용할 수 있습니다. | 엄지·검지·중지를 펴고 약지·새끼를 접어 긴급 상황(자리 비움)을 빠르게 전달합니다. <br/>화면 전체가 반투명 처리되어 "자리 비움" 메시지를 표시합니다. | 엄지만 위로 들어 상대방 의견에 대한 공감·동의를 나타냅니다. <br/>반투명 초록색 배경 위에 "좋은 의견이에요!"를 띄웁니다. | 엄지만 아래로 내려 궁금증 혹은 반대 의견을 표현합니다. <br/>반투명 오렌지색 배경 위에 "질문 있어요!"를 띄웁니다. | 다섯 손가락을 펼쳐 팔레트 모드로 진입합니다. <br/>펜·지우개·색상·크기 선택 등 그림을 그리기 위한 UI가 활성화됩니다. |
...
| **설명**       | **1초**간 버튼 위에 검지 손가락을 올려두면 활성화됩니다. <br/>3초 동안 “안녕하세요!” 메시지와 함께 손 흔드는 큰 이모지가 화면 양옆에 나타납니다. | **1초**간 버튼 위에 검지 손가락을 올려두면 활성화됩니다. <br/>3초 동안 화면 곳곳에 하트 이모지가 떠다니며 감동적인 분위기를 연출합니다.             | **1초**간 버튼 위에 검지 손가락을 올려두면 활성화됩니다. <br/>3초 동안 웃음 이모지가 아래에서 위로 떠올라 유쾌한 분위기를 전달합니다.             | **1초**간 버튼 위에 검지 손가락을 올려두면 활성화됩니다. <br/>3초 동안 놀란 이모지가 파티클처럼 중앙에서 사방으로 퍼져 놀라움을 시각적으로 표현합니다. |
...
| **설명**            | **"All Clear"** 버튼을 1초간 누르면 <br/> 그려진 모든 내용을 즉시 지울 수 있습니다.<br/>검지 손가락이 버튼 위에 있을 때 주황색 게이지가 차오르면 `drawingLayer`가 clear됩니다. | **"pen"** 아이콘을 1초간 누르면 <br/> 펜 모드가 활성화됩니다. <br/> 엄지와 검지를 핀치하면 보라색 원이 표시되고,<br/> 그 경로를 따라 `drawingLayer`에 선이 그려집니다. | **"eraser"** 아이콘을 1초간 누르면 <br/> 지우개 모드가 활성화됩니다. <br/> 엄지와 검지를 핀치하면<br/> 보라색 원 범위만큼 `drawingLayer`가 지워집니다. | **"Color"** 블록을 1초간 누르면 <br/> 4×4 색상 패널이 펼쳐집니다. <br/> 해당 색상 셀 위에 검지 손가락을 1초간 유지하면 <br/> 펜 색상이 바뀝니다. | **"Size : n"** 블록을 1초간 누르면 <br/> 2×8(총 16칸)의 사이즈 패널이 펼쳐집니다. <br/> 셀을 1초간 유지하면 <br/> 펜/지우개의 크기가 1~16 픽셀로 변경됩니다. |
